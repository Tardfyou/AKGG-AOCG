...
class SecurityGraph:
    def __init__(self):
        ...
        self.data_path = os.path.join(cur_dir, 'data/god.json')  # 使用 god.json 作为数据文件
        self.export_dir = os.path.join(cur_dir, 'dict1')  # 数据导出目录
    ...
    def create_graph(self):
        ...
        for item in data:
            ...
            node_properties[event_name] = {
                'description': item.get('description', ''),  # 修正字段名
                'preventive_measures': item.get('prevention_measures', ''),  # 修正字段名
                'attack_reason': item.get('attack_cause', ''),  # 修正字段名
                'defective_software': item.get('defective_device_software', ''),  # 修正字段名
                'detailed_solution': item.get('configuration_solution', ''),  # 修正字段名
            }
